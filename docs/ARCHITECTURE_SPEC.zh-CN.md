...
│  RequestRegistry — 请求生命周期状态机 + 超时扫描         │
├──────────────────────────────────────────────────────┤
│                   传输层 (Transport)                   │
...
└── transport/
    ...
    ├── request_registry.rs     # RequestRegistry（请求生命周期 + 分桶超时扫描）
...
## 请求生命周期与超时治理（Request Lifecycle）

为降低误报并提升高并发稳定性，msgtrans 的请求治理采用“显式状态机 + 统一扫描器”模型。

### 核心原则

1. 请求状态由 `RequestRegistry` 显式管理，不依赖 `Drop` 语义做主判定。  
2. `respond()` 走原子幂等状态迁移，重复响应不再引发日志风暴。  
3. `SessionClosed` 优先终止 pending，请求关闭原因与业务漏响应分离。  
4. 超时检测使用分桶 tick 扫描，避免全量扫描热点。
...
- `TransportContext::respond()`：先执行 `Pending -> Responded` CAS，成功才发送响应。  
- 重复 `respond()`：计入 `duplicate_response_total`，默认 debug 记录。  
- 会话移除时调用 `close_session_pending(session_id)`，批量转 `SessionClosed`。  
- `TransportServer` 启动统一 timeout scanner；每 tick 仅扫描当前桶并二次校验状态。  
- `Drop` 逻辑降级为 debug fallback，不再作为主 warning 机制。
